...
func TestWatchHeartbeat_StaleGoroutine_DoesNotEvictNewSession(t *testing.T) {
	...
	defer done1()
	sess1, err := NewAgentSession(agentUUID, "race-agent", false, conn1)
	require.NoError(t, err)
	...
	defer done2()
	sess2, err := NewAgentSession(agentUUID, "race-agent", false, conn2)
	require.NoError(t, err)
...
func TestWatchHeartbeat_CurrentSession_MarksOfflineAndEvictsFromMap(t *testing.T) {
	...
	sess, err := NewAgentSession(agentUUID, "current-agent", false, conn)
	require.NoError(t, err)
