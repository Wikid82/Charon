...
func TestStartDockerProxy_SetsProxyAddr(t *testing.T) {
	...
	sess, err := NewAgentSession("u1-uuid", "u1-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
...
func TestStartDockerProxy_Idempotent(t *testing.T) {
	...
	sess, err := NewAgentSession("u2-uuid", "u2-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
	defer func() { _ = sess.Close() }()
...
func TestStartDockerProxy_AcceptsAndForwards(t *testing.T) {
	...
	sess, err := NewAgentSession("u3-uuid", "u3-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
	defer func() { _ = sess.Close() }()
...
func TestClose_StopsProxyListener(t *testing.T) {
	...
	sess, err := NewAgentSession("u4-uuid", "u4-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
...
func TestStartDockerProxy_AfterClose(t *testing.T) {
	...
	sess, err := NewAgentSession("u5-uuid", "u5-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
...
func TestAgentSession_runProxyListener_NonErrClosedError_Returns(t *testing.T) {
	...
	sess, err := NewAgentSession("rpl-uuid", "rpl-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
	defer func() { _ = sess.Close() }()
...
func TestAgentSession_proxyConn_ClosedSession_ReturnsQuietly(t *testing.T) {
	...
	sess, err := NewAgentSession("pc-closed-uuid", "pc-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
