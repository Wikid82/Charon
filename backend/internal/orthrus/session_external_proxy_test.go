...
func sessionWithLoopback(t *testing.T) (sess *AgentSession, clientYamux *yamux.Session, cleanup func()) {
	...
	s, err := NewAgentSession("ext-test-uuid", "ext-test-agent", false, nil, serverConn)
	require.NoError(t, err)
...
func TestStartExternalProxy_ZeroPortIsNoOp(t *testing.T) {
	...
	sess, err := NewAgentSession("ext01-uuid", "ext01-agent", false, nil, serverConn)
	require.NoError(t, err)
...
func TestStartExternalProxy_LoopbackNotStarted(t *testing.T) {
	...
	sess, err := NewAgentSession("ext02-uuid", "ext02-agent", false, nil, serverConn)
	require.NoError(t, err)
	defer func() { _ = sess.Close() }()
