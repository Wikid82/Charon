...
func TestOrthrusServer_GetSession_KnownUUID(t *testing.T) {
	...
	sess, err := NewAgentSession("sess-uuid", "sess-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
...
func TestOrthrusServer_GetProxyAddr_SessionExists_NoProxy(t *testing.T) {
	...
	sess, err := NewAgentSession("no-proxy-uuid", "agent", false, nil, nil, serverConn)
	require.NoError(t, err)
	defer func() { _ = sess.Close() }()
...
func TestOrthrusServer_GetProxyAddr_SessionExists_WithProxy(t *testing.T) {
	...
	sess, err := NewAgentSession("with-proxy-uuid", "agent", false, nil, nil, serverConn)
	require.NoError(t, err)
	defer func() { _ = sess.Close() }()
...
func TestOrthrusServer_DisconnectAgent_WithSession(t *testing.T) {
	...
	sess, err := NewAgentSession("disc-uuid", "disc-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
...
func TestOrthrusServer_WatchHeartbeat_ClosedSession_ExitsAndMarksOffline(t *testing.T) {
	...
	sess, err := NewAgentSession("wh-uuid", "wh-agent", false, nil, nil, serverConn)
	require.NoError(t, err)
...
func TestHandleWebSocket_DisplacesExistingSession(t *testing.T) {
	...
	defer oldCleanup()
	oldSess, err := NewAgentSession("displace-uuid", "displace-agent", false, nil, nil, oldConn)
	require.NoError(t, err)
