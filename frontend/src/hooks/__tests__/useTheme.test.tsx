...
describe('useTheme', () => {
  ...
  it('returns the full context shape when used inside ThemeProvider', () => {
    ...
    expect(typeof result.current.setUserTheme).toBe('function')
    expect(Array.isArray(result.current.userThemes)).toBe(true)
  })
  ...
  it('returns null for activeUserTheme when no user theme is active', () => {
    localStorage.clear()
    const { result } = renderHook(() => useTheme(), { wrapper: createWrapper() })
    expect(result.current.activeUserTheme).toBeNull()
  })
})
