...
describe('credentials API', () => {
  ...
  it('should call getCredentials with correct endpoint', async () => {
    const mockData = [mockCredential, { ...mockCredential, uuid: 'test-uuid-2', label: 'Secondary' }]
  ...
  it('should call getCredential with correct endpoint', async () => {
    ...
    const result = await getCredential('provider-uuid-1', 'test-uuid-1')

    expect(client.get).toHaveBeenCalledWith(
      '/dns-providers/provider-uuid-1/credentials/test-uuid-1'
    )
  ...
  it('should call updateCredential with correct endpoint and data', async () => {
    ...
    const result = await updateCredential('provider-uuid-1', 'test-uuid-1', mockCredentialRequest)
  ...
  it('should call deleteCredential with correct endpoint', async () => {
    ...
    await deleteCredential('provider-uuid-1', 'test-uuid-1')
  ...
  it('should call testCredential with correct endpoint', async () => {
    ...
    const result = await testCredential('provider-uuid-1', 'test-uuid-1')

    expect(client.post).toHaveBeenCalledWith(
      '/dns-providers/provider-uuid-1/credentials/test-uuid-1/test'
    )
