...
- **2026-09-04 supply-chain scan triage** (bundled Caddy / CrowdSec / Alpine base — no finding in Charon's own module graph). Full analysis: `docs/security/vulnerability-analysis-2026-09-04.md`
  ...
  - **CVE-2026-84304 / GHSA-vp52-pcj8-j9qc**: gRPC-Go HTTP/2 DATA-frame heap-exhaustion DoS in `google.golang.org/grpc` v1.83.0 in the bundled Caddy binary. The dependency was already pinned to `v1.83.1` (`GRPC_VERSION`, commit `761e37fd`) — the recurrence was a stale `caddy-builder` GHA layer cache in the PR scan-gate build: a global build-arg bump does not reliably invalidate the layer-cache key of a stage that only consumes it, so the `go get grpc@v1.83.1` patch inside the cached-and-skipped `caddy-builder` never re-ran (same edge case as CVE-2026-45135). Fixed by passing `no-cache-filters: caddy-builder,crowdsec-builder` to the `build-charon-image` composite action from `supply-chain-pr.yml` and `security-pr.yml`, matching the release / nightly / e2e builds. The published release image was never affected.
