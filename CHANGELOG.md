...
- **2026-09-04 supply-chain scan triage** (bundled Caddy / CrowdSec / Alpine base — no finding in Charon's own module graph). Full analysis: `docs/security/vulnerability-analysis-2026-09-04.md`
  ...
  - **CVE-2026-84304 / GHSA-vp52-pcj8-j9qc**: gRPC-Go HTTP/2 DATA-frame heap-exhaustion DoS in `google.golang.org/grpc` v1.83.0 (bundled Caddy) — already fixed at source (commit `761e37fd`, `GRPC_VERSION=1.83.1`); the scan ran against a pre-`761e37fd` image and the finding clears on the next image build
